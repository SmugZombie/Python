...
# Imports
import hashlib, json, os, time, sys, _winreg, ConfigParser, urllib2
...
def loadBRConfig():
    ...
    SERVER_KEY = config.get('OSSEC', 'server_key')
    ROOT_DOMAIN = config.get('InstallSettings', 'RootDomain')
...
def writeToFile(file, md5, sha1):
    ...
    outputFile += str(file) + "|" + str(md5) + "|" + str(sha1) + "\n"

def publishConfig():
    url = "https://"+str(ROOT_DOMAIN)+"/api/1.0/fim.json"
    req = urllib2.Request(url, headers={'User-Agent' : "Magic Browser"})
    data = {}
    data['server_key'] = SERVER_KEY
    data['fimdata'] = outputFile
    data['timestamp'] = time.strftime("%b %d %H:%M:%S")

    try:
        response = urllib2.urlopen(req, json.dumps(data), timeout = 30)
